...
export function InlineText({ text, keyPrefix = 'inline' }) {
  return renderInline(text, keyPrefix);
}

export function RichText({ work, text, paragraphClassName }) {
  const source = text != null ? paragraphs(text) : textParagraphs(work);
