...
const Card = ({ title, subtitle, content, itemScope, itemType, itemProp, link }) => {
  return (
    <div className="bg-primary-darkBlue px-8 py-2 rounded-lg shadow-lg mb-4 text-accent-white flex flex-col" itemScope={itemScope} itemType={itemType}>
      <h2 className="text-accent-lightBlue !mt-6 !mb-1" itemProp={itemProp?.title}>
        {link ? <a href={link} className="text-accent-lightBlue">{title}</a> : title}
      </h2>
